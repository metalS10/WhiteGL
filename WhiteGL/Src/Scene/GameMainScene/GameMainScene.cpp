...
bool CGameMain::stageChangeInit()
{
	...
	for (int i = 0; i <= 5; i++)
	{
		...
		for (int j = 0; j <= 10; j++)
		{
			...
			m_game.setupPoly(CVec4(m_polyRange*j, m_polyRange * i, range, range), CVec4(100.0f, r, r, 100.0f), i * 2.0f);	//背景に反映
			...
			m_game.setupPoly(CVec4(m_polyRange*j + m_polyRange * 0.5f, range * 0.5f + (m_polyRange * i), range, -range), CVec4(r, r, 100.0f, 100.0f), (i + 0.5f) * 2.0f);		//背景に反映
		}
	}
	return true;
}
