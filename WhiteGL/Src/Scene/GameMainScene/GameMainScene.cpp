	...
	for (int i = 0; i < m_trianglesLeft.size();i++)
	{
		...
		if (m_trianglesLeft[i] >= posX)
		{
			...
			while (l >= 0)
			{
				...
				if (l <= -1)
				{
					m_game.setPosTrianglesPoly(m_trianglesLeft[i], m_stage->getChangePositionColor(false), i);	//背景に反映
					break;
				}
				else if(l <= 0)
				{
					m_game.setPosTrianglesPoly(m_trianglesLeft[i], m_stage->getChangePositionColor(true), i);	//背景に反映
					break;
				}
			}
		}
