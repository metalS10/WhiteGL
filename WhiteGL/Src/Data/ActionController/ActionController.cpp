...
*@param	速度
*/
CActionMoveStraight::CActionMoveStraight(cocos2d::Point basePos, float vel)
	:m_basePos(basePos), m_vel(vel)
{
	this->start();
}

CActionMoveStraight::CActionMoveStraight(cocos2d::Point basePos, float velX,float velY)
...
*@param	速度
*/
CActionCurve::CActionCurve(cocos2d::Point basePos, float vel)
	:m_basePos(basePos), m_vel(vel)
{
	this->start();
}
CActionCurve::CActionCurve(cocos2d::Point basePos, float velX,float velY)
	: m_basePos(basePos), m_vel(velX),m_velY(velY)
{
	this->start();
}
	...
	if (pChara->m_isAlive)
	{
		...
		*/
		...
		if (this->m_shotCount == 0)
		{
			...
			//敵出撃データを作成
			//CBulletLaunchData* pLaunchData = new CBulletLaunchData(BULLET_TYPE::NORMAL, pChara->m_pMove->m_pos, pChara->m_shotLaunchVector);
			cocos2d::Vec2 vec = cocos2d::Vec2(pChara->m_moveVector.x * 3, pChara->m_moveVector.y);
	...
	if (pChara->m_isAlive)
	{
		if (this->counter <= 0)
		{
			...
			if (pChara->m_pMove->m_pos.x < m_pPlayer->m_pMove->m_pos.x - 10)
			{
				...
				pChara->m_moveVector = cocos2d::Vec2(20, 0);
