...
void CStage1_1::init(CGameEngine& game)
{
	...
	for (int i = 0; i <= 5; i++)
	{
		...
		for (int j = 0; j <= 10; j++)
		{
			...
			game.setupPoly(CVec4(m_polyRange*j, m_polyRange * i, range, range), CVec4(100.0f, r, r, 100.0f), i * 2.0f);	//背景に反映
...
void CStage1_2::init(CGameEngine& game)
{
	...
	for (int i = 0; i <= 5; i++)
	{
		...
		for (int j = 0; j <= 10; j++)
		{
			...
			m_trianglesLeft.push_back(m_polyRange*j);	//このポリゴンのx軸座標左を設定
			m_trianglesRight.push_back((m_polyRange*j) + m_polyRange);	//このポリゴンのx軸座標右を設定
			game.setupPoly(CVec4(m_polyRange*j, m_polyRange * i, range, range), CVec4(r, r, r, 100.0f), i * 2.0f);	//背景に反映
